...
- 2026-09-20 — **v3.42（本地，未部署）信息卡 OpenAI 化**：用户反馈地点详情按钮"橙蓝绿蓝粉五彩渐变太难看"。拆解定因：①色彩彩虹化（5 种饱和渐变）；②无主次层级（三个同级实心按钮）；③拟物质感（渐变+内高光+文字阴影）；④墨石化漏网（info-modal 的 .btn-nav/.btn-amap/.btn-share/.btn-edit 未纳入 v3.29 覆盖清单）。改造（对照 ChatGPT）：到这里去=墨色实心（唯一主操作，暗色浅墨反色）；高德/分享/编辑=幽灵按钮（透明+细描边，hover 中性水洗）；删除=红字幽灵（暗色浅红 #FF8A80）；分类标签=中性灰药丸；7 处 emoji → iconsax 线性图标（location-tick/map-1/share/edit-2/trash/note-2/warning-2）。资源 ?v=109。
- 2026-09-20 — **v3.43（本地，未部署）信息卡卡片与胶囊按钮**：用户要求"卡片、胶囊按钮"继续 OpenAI 化。①卡片去玻璃化：`.modal-content` 由 `rgba(255,255,255,.72)+backdrop-blur+光泽描边`改为**纯白/#1C1C21 纯色** + 24px 圆角 + 干净双层阴影，`::before` 光泽边移除；②内嵌卡片（详细信息/我的备注）改**浅灰填充**（#F7F7F8 / #232328）、无边框；③顶部图标块与关闭按钮中性化（关闭按钮 hover 由品牌橙改中性）；④按钮全圆角**胶囊形**（999px），移除 `.info-actions button::after` 底部拟物光泽层。资源 ?v=110。
- 2026-09-20 — **v3.44（本地，未部署）胶囊凸显**：用户反馈"胶囊凸显不出（透明幽灵形态轮廓不可见）"。修复：次要按钮（高德/分享/编辑）由透明幽灵改为**灰色实心胶囊**（#F2F2F4 / 深色 #2A2A2E），删除按钮改浅红实心胶囊（hover/armed 加深），并统一左右内边距 18px。最终三级按钮体系：黑主胶囊 → 灰次胶囊 → 浅红危险胶囊（全部 999px），即 ChatGPT 按钮标准。资源 ?v=111。
